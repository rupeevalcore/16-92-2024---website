...
try {
    ...
    const lines = data.split('\n'); // Standard newline, works for LF or CRLF usually if split correctly, or use /\r?\n/
    // To preserve line endings, better to split by regex but simplicity:
    ...
    console.log(`Start content: ${startContent.substring(0, 50)}`);
    console.log(`End content: ${endContent.substring(0, 50)}`);

    // Check if start content looks like CSS comment start
    if (!startContent.includes("PROFESSIONAL MOBILE HEADER")) {
        console.warn("WARNING: Start content does not match expected comment!");
    }
